...
(function(Kube)
{
    Kube.Slug = SuperKube.plugin('slug', {
    	...
    	{
        	return value.toLowerCase().replace(/[\s_]/gi, '-').replace(/[\.,\/#!$%\^&\*;:"â€œâ€'{}=_`~()]/g, '');
    	},
    	pass: function()
    	{
        	var value = this.$element.val();
...
(function(Kube)
{
    Kube.Validate = SuperKube.plugin('validate', {
    	...
    	{
        	$('#' + name + '-validation-error').addClass(this.opts.errorClass).text(text).show();
    	},
    	...
    	{
    		$('#' + $(el).attr('name') + '-validation-error').removeClass(this.opts.errorClass).html('').hide();
